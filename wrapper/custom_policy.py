...
from wrapper.manifolds import Sphere


def make_proba_distribution(
    riemannian_wrapper: None, action_space: gym.spaces.Space,
    use_sde: bool = False, dist_kwargs: Optional[Dict[str, Any]] = None
) -> Distribution:
    """
    ...
    if riemannian_wrapper:
        return RiemannianGaussianDistribution(riemannian_wrapper, get_action_dim(action_space), **dist_kwargs)
...
class CustomActorCriticPolicy(BasePolicy):
    ...
    def __init__(
        ...
        # Action distribution
        self.action_dist = make_proba_distribution(manifolds, action_space, use_sde=use_sde, dist_kwargs=dist_kwargs)
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        if "Riemannian" in str(self.action_dist):
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        values = self.value_net(latent_vf)
        distribution = self._get_action_dist_from_latent(latent_pi, latent_sde=latent_sde)
        actions = distribution.get_actions(obs[:, -4:-1], deterministic=deterministic)
        #actions = distribution.get_actions(deterministic=deterministic)
    ...
    def _predict(self, observation: th.Tensor, deterministic: bool = False) -> th.Tensor:
        ...
        latent_pi, _, latent_sde = self._get_latent(observation)
        distribution = self._get_action_dist_from_latent(latent_pi, latent_sde)
        return distribution.get_actions(observation[:, -4:-1], deterministic=deterministic)
        #return distribution.get_actions(deterministic=deterministic)
    ...
    def evaluate_actions(self, obs: th.Tensor, actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        latent_pi, latent_vf, latent_sde = self._get_latent(obs)
        mean_actions = self.action_net(latent_pi)
        mani_mean_actions = self.action_dist.manifolds.log(mean_actions, actions)
        action_std = th.ones_like(mean_actions) * self.log_std.exp()
        distribution = self.action_dist.proba_distribution_eval(th.zeros_like(mean_actions), action_std)
        log_prob = distribution.log_prob(obs[:, -4:-1], mani_mean_actions)
        values = self.value_net(latent_vf)
        return values, log_prob, distribution.entropy()
        '''