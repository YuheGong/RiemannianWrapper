...
class RWPPO(PPO):
    ...
    def __init__(
        ...
        #self.policy.action_dist = RiemannianGaussianDistribution
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            '''Riemannian Wrapper
            x = env.envs[0].env.env.sim.data.mocap_pos.copy()
            u = actions[:, :3].copy()
            u_bar = u / np.linalg.norm(u)
            actions[:, :3] = x * np.cos(np.linalg.norm(u)) + u_bar * np.sin(np.linalg.norm(u))
            '''
